 ...
 * crash dump straight into the main log so it is all in one file. */
...
void crash_log_open(void) {
    ...
    if (!fs) return;
    fsFsCreateFile(fs, "/switch/pvz2_nx/pvz2_crash.log", 0, 0);   /* no-op if exists */
    if (R_SUCCEEDED(fsFsOpenFile(fs, "/switch/pvz2_nx/pvz2_crash.log",
                                 FsOpenMode_Write, &g_crash_file))) {
        /* A normal run leaves a genuinely empty text file. */
