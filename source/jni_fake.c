...
static jlong jni_catch_all(void) {
  static uintptr_t seen_callers[16];
  static unsigned seen_count;
  const uintptr_t caller = (uintptr_t)__builtin_return_address(0);
  const uintptr_t off = pvz2_startup_profile_native_offset(caller);
  int seen = 0;
  for (unsigned i = 0; i < seen_count; ++i)
    if (seen_callers[i] == caller) { seen = 1; break; }
  if (!seen && seen_count < sizeof(seen_callers) / sizeof(seen_callers[0])) {
    seen_callers[seen_count++] = caller;
    if (off != UINTPTR_MAX)
      debugPrintf("JNI catch-all invoked: unimplemented slot caller=libPVZ2+0x%lx\n",
                  (unsigned long)off);
    else
      debugPrintf("JNI catch-all invoked: unimplemented slot caller=%p\n", (void *)caller);
  }
  return 0;
}
