...
int so_load(so_module *mod, const char *filename, void *base, size_t max_size) {
  ...
  debugPrintf("%s: load_base=%p load_virtbase=%p size=%u KB\n", filename,
              mod->load_base, mod->load_virtbase,
...
#define PVZ2_DEFERRED_INIT_CAPACITY 32
static Pvz2DeferredInit pvz2_deferred_inits[PVZ2_DEFERRED_INIT_CAPACITY];
static size_t pvz2_deferred_init_count;

size_t so_pvz2_deferred_init_count(void) {
  return pvz2_deferred_init_count;
}

int so_pvz2_deferred_init_get(size_t index, int *slot, uintptr_t *target) {
  if (index >= pvz2_deferred_init_count) return 0;
  if (slot) *slot = pvz2_deferred_inits[index].slot;
  if (target) *target = pvz2_deferred_inits[index].target;
  return 1;
}
  ...
  for (int i = 0; i < mod->elf_hdr->e_shnum; i++) {
    ...
    if (strcmp(sh_name, ".init_array") == 0) {
      ...
      for (size_t j = 0; j < mod->sec_hdr[i].sh_size / 8; j++) {
        /* Final Android/Nimble initializers remain quarantined here.
         * Slot 1175 is the stock Nexus native-component registrar, but it calls
         * EA::Nimble::findClass() and therefore cannot run until libNimble's
         * JNI_OnLoad has received the JavaVM. main.c invokes that one exact
         * registrar later, after Nimble JNI_OnLoad and before PVZ2 JNI_OnLoad. */
        if (strcmp(mod->name, SO_NAME) == 0 && j >= 1175) {
          if (pvz2_deferred_init_count < PVZ2_DEFERRED_INIT_CAPACITY) {
            Pvz2DeferredInit *entry =
                &pvz2_deferred_inits[pvz2_deferred_init_count++];
            entry->slot = (int)j;
            entry->target = (uintptr_t)init_array[j];
          }
          continue;
        }
        if (init_array[j] != 0)
