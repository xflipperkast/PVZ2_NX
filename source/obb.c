...
static u64 g_startup_obb_decode_ticks;
static unsigned g_startup_obb_reads;
static unsigned g_startup_obb_cache_hits;
#endif
...
void *obb_read(const char *name, size_t *out_size) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (pvz2_startup_profile_active()) g_startup_obb_reads++;
#endif
  ...
  if (cached) {
#if PVZ2_ENABLE_STARTUP_PROFILER
    if (pvz2_startup_profile_active()) g_startup_obb_cache_hits++;
#endif
...
#if PVZ2_ENABLE_STARTUP_PROFILER
  const u64 startup_asset_tick = armGetSystemTick();
#endif
  ...
  if (pvz2_startup_profile_active()) {
    const u64 elapsed_ticks = armGetSystemTick() - startup_asset_tick;
    g_startup_obb_decode_ticks += elapsed_ticks;
    const u64 hz = armGetSystemTickFreq();
    const unsigned long long ms = hz
        ? (unsigned long long)(elapsed_ticks * 1000u / hz) : 0;
    if (ms >= 25)
      debugPrintf("STARTUPPROF +%llums ASSET-SLOW duration=%llums size=%u packed=%u path=%s\n",
                  pvz2_startup_profile_elapsed_ms(), ms, size, packed_size, name);
  }
#endif
  return result;
}
...
void obb_startup_profile_snapshot(const char *tag) {
  ...
  const u64 hz = armGetSystemTickFreq();
  const unsigned long long decode_ms = hz
      ? (unsigned long long)((g_startup_obb_decode_ticks * 1000ULL) / hz) : 0;
  debugPrintf("STARTUPPROF +%llums OBB-SUMMARY tag=%s reads=%u cache_hits=%u decode_ms=%llums resident=%zu\n",
              pvz2_startup_profile_elapsed_ms(), tag ? tag : "?",
              g_startup_obb_reads, g_startup_obb_cache_hits, decode_ms,
              g_hot_asset_bytes);
#else
  (void)tag;
#endif
