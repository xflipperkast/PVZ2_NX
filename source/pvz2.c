...
static u64 startup_profile_start_tick;
static u64 startup_profile_last_tick;
static int startup_profile_running;

static unsigned long long startup_profile_ticks_ms(u64 ticks) {
  const u64 hz = armGetSystemTickFreq();
  return hz ? (unsigned long long)(ticks * 1000u / hz) : 0;
}

#endif

void pvz2_startup_profile_begin(void) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  startup_profile_start_tick = armGetSystemTick();
  startup_profile_last_tick = startup_profile_start_tick;
  startup_profile_running = 1;
  debugPrintf("STARTUPPROF +0ms BEGIN\n");
#endif
}

int pvz2_startup_profile_active(void) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  return startup_profile_running;
#else
  return 0;
#endif
}

unsigned long long pvz2_startup_profile_elapsed_ms(void) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (!startup_profile_start_tick) return 0;
  return startup_profile_ticks_ms(armGetSystemTick() - startup_profile_start_tick);
#else
  return 0;
#endif
}

void pvz2_startup_profile_mark(const char *name) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (!startup_profile_running) return;
  const u64 now = armGetSystemTick();
  const unsigned long long total = startup_profile_ticks_ms(now - startup_profile_start_tick);
  const unsigned long long delta = startup_profile_ticks_ms(now - startup_profile_last_tick);
  startup_profile_last_tick = now;
  debugPrintf("STARTUPPROF +%llums delta=%llums MARK %s\n", total, delta,
              name ? name : "?");
#else
  (void)name;
#endif
...
void pvz2_startup_profile_finish(void) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (!startup_profile_running) return;
  debugPrintf("STARTUPPROF +%llums END loading-finished\n",
              pvz2_startup_profile_elapsed_ms());
  startup_profile_running = 0;
  debugLogFlush();
#endif
}
...
static int synergy_bootstrap_cache_ready;
...
typedef struct {
  ...
  int body_length;
  u64 startup_profile_start_tick;
} HttpRequest;
...
#if PVZ2_ENABLE_STARTUP_PROFILER
static const char *startup_profile_http_kind(HttpKind kind) {
  switch (kind) {
    case HTTP_ANDROID: return "android";
    case HTTP_GLU: return "glu";
    case HTTP_DOWNLOAD: return "download";
    case HTTP_SYNERGY_DIRECTOR: return "synergy-director";
    case HTTP_SYNERGY_BOOTSTRAP_DIRECTION: return "synergy-direction";
    case HTTP_SYNERGY_BOOTSTRAP_DEVICE_ID: return "synergy-device-id";
    case HTTP_SYNERGY_BOOTSTRAP_ANON_UID: return "synergy-anon";
    case HTTP_MTX: return "mtx";
    case HTTP_ACCOUNT_REGISTER: return "account";
    default: return "unknown";
  }
}

static void startup_profile_safe_url(const char *url, char *out, size_t cap) {
  if (!out || !cap) return;
  if (!url) { snprintf(out, cap, "?"); return; }
  size_t n = 0;
  while (url[n] && url[n] != '?' && url[n] != '#' && n + 1 < cap) {
    out[n] = url[n];
    ++n;
  }
  out[n] = 0;
...
void pvz2_startup_profile_snapshot(int loading_state) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (!startup_profile_running) return;
  debugPrintf("STARTUPPROF +%llums HEARTBEAT loading=%d http=%d "
              "synergy{cache=%d update=%d dir=%d/%d dev=%d/%d anon=%d/%d}\n",
              pvz2_startup_profile_elapsed_ms(), loading_state, http_request_count,
              synergy_bootstrap_cache_ready, synergy_update_in_progress,
              synergy_direction_pending || synergy_direction_in_flight,
              synergy_direction_ready,
              synergy_device_id_pending || synergy_device_id_in_flight,
              synergy_device_id_done,
              synergy_anon_pending || synergy_anon_in_flight, synergy_anon_done);
#else
  (void)loading_state;
#endif
}
...
static int http_request_start(HttpRequest *request, const char *url,
  ...
  request->startup_profile_start_tick = armGetSystemTick();
  http_requests[http_request_count++] = request;
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (startup_profile_running) {
    char safe_url[384];
    startup_profile_safe_url(request->url, safe_url, sizeof(safe_url));
    debugPrintf("STARTUPPROF +%llums HTTP-BEGIN kind=%s timeout=%ld url=%s\n",
                pvz2_startup_profile_elapsed_ms(),
                startup_profile_http_kind(request->kind), timeout_ms, safe_url);
  }
#endif
  return 1;
}
...
#define SYNERGY_BOOTSTRAP_CACHE_SCOPE "pvz2_nx.synergyBootstrap"
#define SYNERGY_BOOTSTRAP_CACHE_MAX_AGE_SECONDS (7LL * 24LL * 60LL * 60LL)

static int synergy_parse_direction_state(const unsigned char *data,
                                         size_t length) {
  synergy_hw_id[0] = 0;
  ...
  return synergy_json_scalar_string(data, length, "hwId", synergy_hw_id,
                                    sizeof(synergy_hw_id)) &&
         synergy_server_url(data, length, "synergy.user", synergy_user_url,
                            sizeof(synergy_user_url));
}

static int synergy_restore_bootstrap_cache(void) {
  const char *version = store_get_raw(property_store, 64,
                                      SYNERGY_BOOTSTRAP_CACHE_SCOPE, "version");
  const char *locale = store_get_raw(property_store, 64,
                                     SYNERGY_BOOTSTRAP_CACHE_SCOPE, "locale");
  const char *saved_epoch = store_get_raw(property_store, 64,
                                          SYNERGY_BOOTSTRAP_CACHE_SCOPE,
                                          "savedEpoch");
  const char *direction = store_get_raw(property_store, 64,
                                        SYNERGY_BOOTSTRAP_CACHE_SCOPE,
                                        "directionJson");
  const char *device_id = store_get_raw(property_store, 64,
                                        SYNERGY_BOOTSTRAP_CACHE_SCOPE,
                                        "eaDeviceId");
  if (!version || strcmp(version, GAME_VERSION) || !locale || !saved_epoch ||
      !direction || !*direction || !device_id || !*device_id)
    return 0;

  const SwitchLocaleInfo current_locale = get_switch_locale();
  if (strcmp(locale, current_locale.locale)) return 0;

  char *end = NULL;
  const long long stored = strtoll(saved_epoch, &end, 10);
  const long long now = (long long)time(NULL);
  if (!end || *end || stored <= 0 || now <= 0 || stored > now + 300 ||
      now - stored > SYNERGY_BOOTSTRAP_CACHE_MAX_AGE_SECONDS)
    return 0;

  const size_t length = strlen(direction);
  if (!length || length > 4u * 1024u * 1024u) return 0;

  synergy_capture_direction_response((const unsigned char *)direction, length);
  if (!synergy_direction_json ||
      !synergy_parse_direction_state((const unsigned char *)direction, length)) {
    free(synergy_direction_json);
    synergy_direction_json = NULL;
    synergy_direction_json_length = 0;
    return 0;
  ...
  snprintf(synergy_ea_device_id, sizeof(synergy_ea_device_id), "%s", device_id);
  synergy_direction_ready = 1;
  synergy_device_id_done = 1;
  return 1;
}

static void synergy_save_bootstrap_cache(void) {
  if (!synergy_direction_json || !synergy_direction_json_length ||
      !synergy_ea_device_id[0])
    return;

  const SwitchLocaleInfo locale = get_switch_locale();
  char saved_epoch[32];
  snprintf(saved_epoch, sizeof(saved_epoch), "%lld", (long long)time(NULL));
  ...
  const struct {
    ...
    const char *value;
      ...
      {"savedEpoch", saved_epoch},
      {"directionJson", synergy_direction_json},
      {"eaDeviceId", synergy_ea_device_id},
  };

  for (size_t i = 0; i < sizeof(entries) / sizeof(entries[0]); ++i) {
    const int rc = store_set_raw(property_store, 64,
                                 SYNERGY_BOOTSTRAP_CACHE_SCOPE,
                                 entries[i].key, entries[i].value);
    if (rc < 0) return;
    if (rc > 0) changed = 1;
  }
  if (changed) (void)java_store_save();
}
...
static void synergy_direction_complete(const HttpRequest *request,
  ...
  if (!ok || !synergy_parse_direction_state(data, length)) {
...
static void synergy_java_setup(void) {
  ...
  synergy_bootstrap_cache_ready = synergy_restore_bootstrap_cache();
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (startup_profile_running)
    debugPrintf("STARTUPPROF +%llums SYNERGY-CACHE %s\n",
                pvz2_startup_profile_elapsed_ms(),
                synergy_bootstrap_cache_ready ? "hit" : "miss");
#endif
}
...
static void synergy_java_restore(void) {
  ...
  if (synergy_bootstrap_cache_ready) {
    synergy_anon_pending = 1;
    if (synergy_start_synergy_getanonuid()) return;
    synergy_bootstrap_cache_ready = 0;
  }
  synergy_direction_pending = 1;
...
static void http_complete(HttpRequest *request, CURLcode code) {
  ...
  if (startup_profile_running) {
    const u64 now = armGetSystemTick();
    const unsigned long long duration = request->startup_profile_start_tick
        ? startup_profile_ticks_ms(now - request->startup_profile_start_tick) : 0;
    char safe_url[384];
    startup_profile_safe_url(request->url, safe_url, sizeof(safe_url));
    debugPrintf("STARTUPPROF +%llums HTTP-END duration=%llums kind=%s curl=%d "
                "status=%ld bytes=%zu url=%s\n",
                pvz2_startup_profile_elapsed_ms(), duration,
                startup_profile_http_kind(request->kind), (int)code, status,
                request->response.size, safe_url);
  }
#endif
