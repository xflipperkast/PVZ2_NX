...
void debugLogCheckpoint(void);
void debugLogSync(void);
#else
/* Keep disabled log arguments visible to the compiler for format/type checking
 * and to avoid debug-only locals becoming "unused", but never evaluate them
 * or emit a runtime call.  GCC/Clang fold the constant-false branch away. */
static inline int debugPrintf_disabled(const char *text, ...)
    __attribute__((format(printf, 1, 2)));
static inline int debugPrintf_disabled(const char *text, ...) {
  (void)text;
  return 0;
}
#define debugPrintf(...) ({ if (0) debugPrintf_disabled(__VA_ARGS__); 0; })
#define debugLogFlush() ((void)0)
#define debugLogCheckpoint() ((void)0)
#define debugLogSync() ((void)0)
#endif
