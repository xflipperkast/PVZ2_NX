...
GLint glGetUniformLocation_fake(GLuint program, const GLchar *name) {
  ...
  if (PVZ2_ENABLE_GL_TRACE && location >= 0 && name && !strcmp(name, "screenMatrix"))
...
void glVertexAttribPointer_fake(GLuint index, GLint size, GLenum type,
  ...
  if (index < sizeof(g_vertex_attribs) / sizeof(g_vertex_attribs[0]))
    g_vertex_attribs[index] = (VertexAttribInfo){ size, type, normalized, stride, pointer };
...
void cursor_draw(void) {
  ...
  GLint  attr0_on = 0, attr0_buf = 0, attr0_size = 4, attr0_stride = 0;
  GLint  attr0_type = GL_FLOAT, attr0_normalized = GL_FALSE;
  void  *attr0_ptr = NULL;
  ...
  glGetVertexAttribiv(0, GL_VERTEX_ATTRIB_ARRAY_BUFFER_BINDING, &attr0_buf);
  ...
  glBindBuffer(GL_ARRAY_BUFFER, (GLuint)attr0_buf);
  glVertexAttribPointer(0, attr0_size, (GLenum)attr0_type,
                        (GLboolean)attr0_normalized, attr0_stride, attr0_ptr);
  if (attr0_on) glEnableVertexAttribArray(0);
