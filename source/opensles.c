...
static u64 g_startup_audio_device_ticks;
static u64 g_startup_audio_player_ticks;
static unsigned g_startup_audio_device_count;
static unsigned g_startup_audio_player_count;
static u64 g_startup_audio_register_ticks;
static u64 g_startup_audio_register_kick_ticks;
static u64 g_startup_audio_setplay_ticks;
static u64 g_startup_audio_setplay_kick_ticks;
static u64 g_startup_audio_enqueue_ticks;
static u64 g_startup_audio_pump_ticks;
static u64 g_startup_audio_pump_callback_ticks;
static unsigned g_startup_audio_register_count;
static unsigned g_startup_audio_register_kick_count;
static unsigned g_startup_audio_setplay_count;
static unsigned g_startup_audio_setplay_kick_count;
static unsigned g_startup_audio_enqueue_count;
static unsigned g_startup_audio_pump_count;
static unsigned g_startup_audio_pump_callback_count;

static unsigned long long opensles_profile_ticks_ms(u64 ticks) {
...
void opensles_startup_profile_snapshot(const char *tag) {
  ...
  debugPrintf("STARTUPPROF +%llums AUDIO-SUMMARY tag=%s device=%u/%llums players=%u/%llums register=%u/%llums kick=%u/%llums setplay=%u/%llums kick=%u/%llums enqueue=%u/%llums pump=%u/%llums callback=%u/%llums\n",
              pvz2_startup_profile_elapsed_ms(), tag ? tag : "?",
              g_startup_audio_device_count, opensles_profile_ticks_ms(g_startup_audio_device_ticks),
              g_startup_audio_player_count, opensles_profile_ticks_ms(g_startup_audio_player_ticks),
              g_startup_audio_register_count, opensles_profile_ticks_ms(g_startup_audio_register_ticks),
              g_startup_audio_register_kick_count, opensles_profile_ticks_ms(g_startup_audio_register_kick_ticks),
              g_startup_audio_setplay_count, opensles_profile_ticks_ms(g_startup_audio_setplay_ticks),
              g_startup_audio_setplay_kick_count, opensles_profile_ticks_ms(g_startup_audio_setplay_kick_ticks),
              g_startup_audio_enqueue_count, opensles_profile_ticks_ms(g_startup_audio_enqueue_ticks),
              g_startup_audio_pump_count, opensles_profile_ticks_ms(g_startup_audio_pump_ticks),
              g_startup_audio_pump_callback_count, opensles_profile_ticks_ms(g_startup_audio_pump_callback_ticks));
#else
  (void)tag;
#endif
}
...
void opensles_pump(void) {
  ...
  if (!g_dev)
    return;
#if PVZ2_ENABLE_STARTUP_PROFILER
  const int startup_profile = pvz2_startup_profile_active();
#endif
  ...
  for (;;) {
...
#if PVZ2_ENABLE_STARTUP_PROFILER
      if (startup_profile) { g_startup_audio_pump_count++; g_startup_audio_pump_ticks += armGetSystemTick() - profile_begin; }
#endif
      return;
    }
    if (!released || !count) {
    ...
    #if PVZ2_ENABLE_STARTUP_PROFILER
    const u64 callback_begin = armGetSystemTick();
    #endif
    audio_callback(NULL, released->buffer, OUT_BYTES);
#if PVZ2_ENABLE_STARTUP_PROFILER
    if (startup_profile) {
      g_startup_audio_pump_callback_count++;
      g_startup_audio_pump_callback_ticks += armGetSystemTick() - callback_begin;
    }
#endif
...
static void ensure_device(int rate) {
  ...
  if (pvz2_startup_profile_active()) {
    g_startup_audio_device_count++;
    g_startup_audio_device_ticks += armGetSystemTick() - profile_begin;
  }
#endif
...
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (pvz2_startup_profile_active()) {
    g_startup_audio_enqueue_count++;
    g_startup_audio_enqueue_ticks += armGetSystemTick() - profile_begin;
  }
#endif
...
static SLresult bq_RegisterCallback(void *self, slBufferQueueCallback cb, void *ctx) {
  ...
  if (cb && kick) {
    #if PVZ2_ENABLE_STARTUP_PROFILER
    const u64 kick_begin = armGetSystemTick();
    #endif
    cb(&p->bq_vt, ctx);
#if PVZ2_ENABLE_STARTUP_PROFILER
    if (pvz2_startup_profile_active()) {
      g_startup_audio_register_kick_count++;
      g_startup_audio_register_kick_ticks += armGetSystemTick() - kick_begin;
    }
#endif
  }
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (pvz2_startup_profile_active()) {
    g_startup_audio_register_count++;
    g_startup_audio_register_ticks += armGetSystemTick() - profile_begin;
  }
#endif
  return SL_RESULT_SUCCESS;
}
...
#if PVZ2_ENABLE_STARTUP_PROFILER
    if (pvz2_startup_profile_active()) {
      g_startup_audio_setplay_kick_count++;
      g_startup_audio_setplay_kick_ticks += armGetSystemTick() - kick_begin;
    }
#endif
  }
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (pvz2_startup_profile_active()) {
    g_startup_audio_setplay_count++;
    g_startup_audio_setplay_ticks += armGetSystemTick() - profile_begin;
  }
#endif
  return SL_RESULT_SUCCESS;
}
...
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (pvz2_startup_profile_active()) {
    g_startup_audio_player_count++;
    g_startup_audio_player_ticks += armGetSystemTick() - profile_begin;
  }
#endif
  return SL_RESULT_SUCCESS;
}
