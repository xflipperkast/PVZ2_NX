...
void debugLogCheckpoint(void) {
  ...
  log_flush_locked();
  if (s_log_ready == 1) {
    /* Trim the preallocation so the file is immediately useful if copied
     * while the title is still running. No fsFileFlush here: checkpoints are
     * timing instrumentation, not crash-durability barriers. */
