...
int fcntl_fake(int fd, int cmd, ...) {
  enum { BIONIC_O_NONBLOCK_LOCAL = 00004000 };
  va_list ap; va_start(ap, cmd);
  int ret = 0;
  switch (cmd) {
    case F_DUPFD: ret = dup(fd); break;
    case F_GETFL: {
      int host = fcntl(fd, F_GETFL, 0);
      if (host < 0) ret = O_RDWR;
      else {
        ret = host & ~O_NONBLOCK;
        if (host & O_NONBLOCK) ret |= BIONIC_O_NONBLOCK_LOCAL;
      }
      break;
    }
    case F_SETFL: {
      ...
      if (host < 0) ret = -1;
        ...
        if (requested & BIONIC_O_NONBLOCK_LOCAL) host |= O_NONBLOCK;
        else host &= ~O_NONBLOCK;
        ret = fcntl(fd, F_SETFL, host);
      }
      break;
    }
    case F_GETFD: ret = fcntl(fd, F_GETFD, 0); break;
    case F_SETFD: ret = fcntl(fd, F_SETFD, va_arg(ap, int)); break;
    default: ret = 0; break;
  }
  va_end(ap);
  return ret;
...
int mkdir_fake(const char *path, unsigned int mode) {
  /* Map Android root-relative paths into the Switch data sandbox. */
  char mapped[768];
  ...
  int rc = pvz2_mkdir_native_locked(target, mode);
  int saved_errno = errno;
  if (rc != 0 && saved_errno == EEXIST) rc = 0;
  errno = saved_errno;
  return rc;
}
