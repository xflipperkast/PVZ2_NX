...
static void SDLCALL audio_callback(void *ud, Uint8 *stream, int len) {
  ...
  /* Sample diagnostics used to rescan every mixed block even though normal
   * builds logged only one in 300. Decide first, then collect those counters
   * only for the rare block that will actually be printed. */
  ...
  for (int i = 0; i < frames * 2; i++) {
    ...
    if (log_final_mix) {
      if (v) final_nonzero++;
      const int av = v < 0 ? (int)-v : (int)v;
      if (av > final_peak) final_peak = av;
    }
  }
...
static SLresult bq_Enqueue(void *self, const void *pBuffer, SLuint32 size) {
  ...
  const int log_enqueue = enq_count < 10 || (enq_count % 1800) == 0 ||
      input_trace_trace;
  if (log_enqueue) {
    const int16_t *s16 = (const int16_t *)pBuffer;
    int nonzero = 0;
    int max_sample = 0;
    for (size_t i = 0; i < size / 2; i++) {
      const int value = s16[i];
      if (value != 0) nonzero++;
    ...
    debugPrintf("opensles: bq_Enqueue #%u player=%p rate=%d ch=%d playing=%d q=%d "
                ...
                pBuffer, size, nonzero, (size_t)size / 2u, max_sample,
                p->gain,
