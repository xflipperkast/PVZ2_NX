...
/* Startup resource I/O probe + read buffering.
 *
 * Hardware scope timings show the two remaining large startup blocks are both
 * resource-table work: ResourceManager::Init and LoadNetworkTables.  Keep file
 * semantics stock, but give read-only RTON/package streams a larger stdio
 * buffer and account the actual read/seek time so the next hardware run tells
 * us whether the bottleneck is SD I/O or CPU deserialization.
 *
 * The fd map is deliberately tiny and lock-free on the hot read path.  Entries
 * are created only while the startup profiler is active; counters use atomics
 * because PVZ2 may read resources from worker threads as well as the main
 * thread. */
#define STARTUP_IO_MAX_FD 256
#define STARTUP_IO_MAX_FILES 96
#define STARTUP_IO_PATH_LEN 128
#define STARTUP_IO_BUFFER_SIZE (256u * 1024u)

typedef struct {
  char path[STARTUP_IO_PATH_LEN];
  unsigned buffered;
  unsigned fopen_count;
  unsigned fread_calls;
  unsigned read_calls;
  unsigned seek_calls;
...
static StartupIoFile g_startup_io_files[STARTUP_IO_MAX_FILES];
static unsigned g_startup_io_file_count;
/* 0 = untracked; otherwise file-index + 1. */
static unsigned short g_startup_io_fd_map[STARTUP_IO_MAX_FD];
static unsigned g_startup_io_buffered_streams;

static int startup_io_path_is_resource(const char *path) {
  if (!path || !pvz2_startup_profile_active()) return 0;
  const char *ext = strrchr(path, '.');
  if (ext && (!strcasecmp(ext, ".rton") || !strcasecmp(ext, ".rsb") ||
              !strcasecmp(ext, ".rsg") || !strcasecmp(ext, ".pak") ||
              !strcasecmp(ext, ".json")))
    return 1;
  return strstr(path, "/Packages/") || strstr(path, "/packages/") ||
         strstr(path, "/CDN.13.3/");
}
#endif

static int startup_io_path_should_buffer(const char *path) {
  if (!path) return 0;
  const char *ext = strrchr(path, '.');
  if (ext && (!strcasecmp(ext, ".rton") || !strcasecmp(ext, ".rsb") ||
              !strcasecmp(ext, ".rsg") || !strcasecmp(ext, ".pak") ||
...
#if PVZ2_ENABLE_STARTUP_PROFILER
static unsigned startup_io_get_or_add_file(const char *path, int buffered) {
  unsigned result = STARTUP_IO_MAX_FILES;
  rmutexLock(&g_file_lock);
  for (unsigned i = 0; i < g_startup_io_file_count; ++i) {
    if (!strncmp(g_startup_io_files[i].path, path,
                 sizeof(g_startup_io_files[i].path))) {
      result = i;
      break;
    }
  }
  if (result == STARTUP_IO_MAX_FILES &&
      g_startup_io_file_count < STARTUP_IO_MAX_FILES) {
    result = g_startup_io_file_count++;
    snprintf(g_startup_io_files[result].path,
             sizeof(g_startup_io_files[result].path), "%s", path);
  }
  if (result < STARTUP_IO_MAX_FILES) {
    g_startup_io_files[result].fopen_count++;
    if (buffered) g_startup_io_files[result].buffered = 1;
  }
  rmutexUnlock(&g_file_lock);
  return result;
}
#endif

static void startup_io_register_fd(int fd, const char *path, int buffered) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (fd < 0 || fd >= STARTUP_IO_MAX_FD ||
      !startup_io_path_is_resource(path))
    return;
  const unsigned index = startup_io_get_or_add_file(path, buffered);
  if (index < STARTUP_IO_MAX_FILES)
    __atomic_store_n(&g_startup_io_fd_map[fd], (unsigned short)(index + 1),
                     __ATOMIC_RELEASE);
#else
  (void)fd; (void)path; (void)buffered;
#endif
}

static StartupIoFile *startup_io_file_for_fd(int fd) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (fd < 0 || fd >= STARTUP_IO_MAX_FD || !pvz2_startup_profile_active())
    return NULL;
  const unsigned short encoded =
      __atomic_load_n(&g_startup_io_fd_map[fd], __ATOMIC_ACQUIRE);
  if (!encoded || encoded > STARTUP_IO_MAX_FILES) return NULL;
  return &g_startup_io_files[encoded - 1];
#else
  (void)fd;
  return NULL;
#endif
...
static void startup_io_unregister_fd(int fd) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (fd < 0 || fd >= STARTUP_IO_MAX_FD) return;
  const unsigned short encoded =
      __atomic_exchange_n(&g_startup_io_fd_map[fd], 0, __ATOMIC_ACQ_REL);
  if (!encoded || encoded > STARTUP_IO_MAX_FILES) return;
  StartupIoFile *entry = &g_startup_io_files[encoded - 1];
  const u64 ticks = __atomic_load_n(&entry->fread_ticks, __ATOMIC_RELAXED) +
                    __atomic_load_n(&entry->read_ticks, __ATOMIC_RELAXED) +
                    __atomic_load_n(&entry->seek_ticks, __ATOMIC_RELAXED);
  const u64 hz = armGetSystemTickFreq();
  const unsigned long long ms = hz ? (unsigned long long)(ticks * 1000ULL / hz) : 0;
  const unsigned fread_calls = __atomic_load_n(&entry->fread_calls, __ATOMIC_RELAXED);
  const unsigned read_calls = __atomic_load_n(&entry->read_calls, __ATOMIC_RELAXED);
  const unsigned seek_calls = __atomic_load_n(&entry->seek_calls, __ATOMIC_RELAXED);
  if (pvz2_startup_profile_active() &&
      (ms >= 10 || fread_calls + read_calls + seek_calls >= 64)) {
    const char *base = strrchr(entry->path, '/');
    base = base ? base + 1 : entry->path;
    debugPrintf("STARTUPPROF +%llums RESOURCE-IO file=%s buffered=%u fread=%u/%llubytes read=%u/%llubytes seek=%u io_ms=%llums\n",
                pvz2_startup_profile_elapsed_ms(), base, entry->buffered,
                fread_calls,
                __atomic_load_n(&entry->fread_bytes, __ATOMIC_RELAXED),
                read_calls,
                __atomic_load_n(&entry->read_bytes, __ATOMIC_RELAXED),
                seek_calls, ms);
  }
#else
  (void)fd;
...
static inline void file_lock_acquire(void) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  const int profile = pvz2_startup_profile_active();
  const u64 begin = profile ? armGetSystemTick() : 0;
#endif
  rmutexLock(&g_file_lock);
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (profile) {
    const u64 ticks = armGetSystemTick() - begin;
    const u64 hz = armGetSystemTickFreq();
    const unsigned long long ms = hz ? (unsigned long long)((ticks * 1000ULL) / hz) : 0;
    if (ms >= 25)
      debugPrintf("STARTUPPROF +%llums FILE-LOCK-WAIT duration=%llums\n",
                  pvz2_startup_profile_elapsed_ms(), ms);
  }
#endif
}

#define FILE_LOCK()   file_lock_acquire()
...
/* os_shims.c owns the Android-path mkdir wrapper, but the final newlib call
 * must share this exact fd/device-layer lock with open/fopen/close. */
int pvz2_mkdir_native_locked(const char *path, unsigned int mode) {
  return mkdir_L(path, (mode_t)mode);
}
...
/* Android/Linux permits unlinking an open file: the directory entry vanishes
 * immediately while existing descriptors keep the old inode alive. PVZ2's
 * stock CDN TAR worker relies on exactly that behavior for tar.full: libtar
 * keeps the archive descriptor open, then the caller removes tar.full before
 * the TAR object is closed. Horizon's SD filesystem cannot unlink/truncate
 * that still-open file, so the second CDN package fails fopen("tar.full",
 * "wb") with EIO and all later packages fail behind it.
 *
 * Preserve the stock logical path while rotating the Switch backing filename
 * after every logical remove. Old generations may remain open until process
 * exit, just like an unlinked inode on Android, but are no longer reachable by
 * the logical tar.full name. This is filesystem-semantics emulation only; it
 * does not alter CDN success/accounting state. */
#define CDN_TAR_FULL_ANDROID "/No_Backup/CDN.13.3/tar.full"
static unsigned g_cdn_tar_full_generation;

static int path_is_cdn_tar_full_logical(const char *path) {
  if (!path) return 0;
  if (!strcmp(path, CDN_TAR_FULL_ANDROID)) return 1;
  return !strcmp(path, DATA_DIR CDN_TAR_FULL_ANDROID);
}

static unsigned cdn_tar_full_generation_load(void) {
  return __atomic_load_n(&g_cdn_tar_full_generation, __ATOMIC_RELAXED);
}

static unsigned cdn_tar_full_generation_advance(void) {
  return __atomic_add_fetch(&g_cdn_tar_full_generation, 1u, __ATOMIC_RELAXED);
}
...
ssize_t read_fake(int fd, void *buf, size_t count) {
  StartupIoFile *entry = startup_io_file_for_fd(fd);
  const u64 begin = entry ? armGetSystemTick() : 0;
  const ssize_t result = read(fd, buf, count);
  if (entry) {
    __atomic_fetch_add(&entry->read_calls, 1u, __ATOMIC_RELAXED);
    if (result > 0)
      __atomic_fetch_add(&entry->read_bytes, (unsigned long long)result,
                         __ATOMIC_RELAXED);
    __atomic_fetch_add(&entry->read_ticks, armGetSystemTick() - begin,
                       __ATOMIC_RELAXED);
  }
  return result;
}
...
off_t lseek_fake(int fd, off_t offset, int whence) {
  ...
  if (entry) {
    __atomic_fetch_add(&entry->seek_calls, 1u, __ATOMIC_RELAXED);
    __atomic_fetch_add(&entry->seek_ticks, armGetSystemTick() - begin,
                       __ATOMIC_RELAXED);
  }
  return result;
}
...
/*  Android/OpenSSL on arm64 may seed its CSPRNG through the Linux
 * getrandom(2) ABI (directly or via syscall(278)). Horizon has no Linux kernel
 * syscall, but libnx exposes the Switch cryptographic RNG service. Bridge only
 * this entropy primitive to the real service; never synthesize seed bytes. */
ssize_t getrandom_fake(void *buf, size_t buflen, unsigned int flags) {
  enum { BIONIC_GRND_NONBLOCK = 0x1u, BIONIC_GRND_RANDOM = 0x2u };
  static unsigned trace_count;

  if (flags & ~(BIONIC_GRND_NONBLOCK | BIONIC_GRND_RANDOM)) {
    errno = EINVAL;
    return -1;
  }
  if (!buf && buflen) {
  ...
  const Result rc = csrngGetRandomBytes(buf, buflen);
  if (trace_count < 16) {
    ++trace_count;
    (void)0;
  }
...
long syscall_fake(long number, ...) {
  switch (number) {
    ...
    case ARM64_SYS_GETRANDOM: {
      ...
      const size_t buflen = va_arg(va, size_t);
      const unsigned int flags = va_arg(va, unsigned int);
      va_end(va);
      return (long)getrandom_fake(buf, buflen, flags);
    }
  }
...
int open_fake(const char *path, int flags, ...) {
  ...
  if (fd >= 0 && (convert_open_flags(flags) & 3) == O_RDONLY)
    startup_io_register_fd(fd, path, 0);
  return fd;
}

/* Stock ContentDownloader gunzips /No_Backup/CDN.13.3/tar.tmp
 * with zlib gzopen().  Unlike fopen/open, gzopen used to bypass the Android
 * path remapper, so it looked for /No_Backup at the Switch filesystem root
 * and returned NULL.  Keep stock gzread/gzclose authoritative; only translate
 * the pathname into this port's data sandbox. */
void *gzopen_fake(const char *path, const char *mode) {
  ...
  /* Do not call zlib's gzopen(path) here.  gzopen performs its own raw
   * newlib open(), bypassing the port's shared device/fd-table lock.  Stock
   * CDN decompression runs on worker threads, exactly where that bypass can
   * race the main thread's asset I/O.  Open under the established lock, then
   * hand ownership of the fd to zlib with gzdopen(). */
  int flags = O_RDONLY;
  if (mode && mode[0] == 'w') flags = O_WRONLY | O_CREAT | O_TRUNC;
  else if (mode && mode[0] == 'a') flags = O_WRONLY | O_CREAT | O_APPEND;
  const int fd = open_L(path, flags, 0666);
  if (fd < 0) return NULL;

  gzFile file = gzdopen(fd, mode ? mode : "rb");
  if (!file) close_fake(fd); /* gzdopen owns fd only on success. */
  return (void *)file;
}

int gzclose_fake(void *opaque) {
  if (!opaque) return Z_STREAM_ERROR;
  /* gzclose closes its owned fd internally.  Serialize that fd-table mutation
   * with the same lock used by close_fake/open_fake. */
  FILE_LOCK();
  const int rc = gzclose((gzFile)opaque);
...
/* The stock TAR extractor applies POSIX mode bits after writing each member.
 * Switch SD filesystems do not have Android's permission model, but the path
 * must still be translated so a cosmetic chmod cannot turn an otherwise good
 * extraction into an error.  Require that the target exists; then treat
 * unsupported permission metadata as success. */
int chmod_fake(const char *path, unsigned int mode) {
  ...
  FILE_UNLOCK();
  /* FAT/exFAT permission bits are not semantically meaningful for PVZ2.
   * Once the mapped target is proven to exist, a metadata-only chmod failure
   * must not invalidate the extracted payload. */
  const int rc = 0;
  if (rc == 0) errno = native_errno;
  return rc;
}

int openat_fake(int dirfd, const char *path, int flags, ...) {
  /* Android's TAR installer may use openat() relative to a directory fd.
   * Paths are remapped into the Switch data sandbox before opening. */
  (void)dirfd;
  char rbuf[600];
  path = remap_data_path(path, rbuf, sizeof rbuf);
  if (path_is_profile_bundle_trigger(path) && (flags & 3) == O_RDONLY)
...
/* During loading state 3 the stock content-refresh listener scans several
 * thousand possible hotpackage level names synchronously. Most do not exist.
 * The exact stock caller at libPVZ2+0x2405078 consumes only stat() success and,
 * when requested, the file-vs-directory mode bits. Build one read-only snapshot
 * of hotpackagestorage and answer that exact startup call site from RAM. The
 * index is not used by other callers or after state 3, so later downloads keep
 * normal filesystem semantics. */
typedef struct {
  char *path;
  s8 type;
} StartupHotIndexEntry;

typedef struct {
  StartupHotIndexEntry *entries;
  size_t count;
  size_t capacity;
  size_t directories;
  size_t files;
} StartupHotIndexBuild;

static StartupHotIndexEntry *g_startup_hot_index;
static size_t g_startup_hot_index_count;
static int g_startup_hot_index_state; /* 0=unbuilt, 1=building, 2=ready, -1=failed */
static unsigned g_startup_hot_index_hits;
static unsigned g_startup_hot_index_misses;

static int startup_hot_index_push(StartupHotIndexBuild *build,
                                  const char *path, s8 type) {
  if (build->count == build->capacity) {
    size_t next = build->capacity ? build->capacity * 2 : 1024;
    if (next < build->capacity ||
        next > SIZE_MAX / sizeof(StartupHotIndexEntry)) return 0;
    StartupHotIndexEntry *grown = realloc(
        build->entries, next * sizeof(StartupHotIndexEntry));
    if (!grown) return 0;
    build->entries = grown;
    build->capacity = next;
  }
  ...
  if (!copy) return 0;
  build->entries[build->count].path = copy;
  build->entries[build->count].type = type;
  build->count++;
  return 1;
}

static int startup_hot_index_scan_dir(FsFileSystem *fs, const char *dir_path,
                                      StartupHotIndexBuild *build) {
  FsDir dir;
  Result rc = fsFsOpenDirectory(fs, dir_path,
  ...
  while (ok) {
    s64 total = 0;
    rc = fsDirRead(&dir, &total, 16, entries);
    if (R_FAILED(rc)) { ok = 0; break; }
    if (total <= 0) break;
    for (s64 i = 0; i < total; ++i) {
      const char *name = entries[i].name;
      if (!name[0] || !strcmp(name, ".") || !strcmp(name, "..")) continue;
      char child[FS_MAX_PATH];
      const int length = snprintf(child, sizeof(child), "%s/%s", dir_path, name);
      if (length <= 0 || (size_t)length >= sizeof(child)) { ok = 0; break; }
      if (!startup_hot_index_push(build, child, entries[i].type)) {
        ok = 0;
        break;
      }
      if (entries[i].type == FsDirEntryType_Dir) {
        build->directories++;
        if (!startup_hot_index_scan_dir(fs, child, build)) { ok = 0; break; }
      } else {
        build->files++;
      }
    }
...
static int startup_hot_entry_compare(const void *lhs, const void *rhs) {
  const StartupHotIndexEntry *a = (const StartupHotIndexEntry *)lhs;
  const StartupHotIndexEntry *b = (const StartupHotIndexEntry *)rhs;
  return strcasecmp(a->path, b->path);
}

static void startup_hot_index_build_free(StartupHotIndexBuild *build) {
  if (!build) return;
  for (size_t i = 0; i < build->count; ++i) free(build->entries[i].path);
  free(build->entries);
  memset(build, 0, sizeof(*build));
}

static int startup_hot_index_build(FsFileSystem *fs) {
  static const char hot_fs_root[] = DATA_DIR "/No_Backup/hotpackagestorage";
  const char *root = hot_fs_root;
  if (!strncmp(root, "sdmc:", 5)) root += 5;

  StartupHotIndexBuild build;
  memset(&build, 0, sizeof(build));
#if PVZ2_ENABLE_STARTUP_PROFILER
  const u64 begin = armGetSystemTick();
#endif
  int ok = startup_hot_index_push(&build, root, FsDirEntryType_Dir) &&
           startup_hot_index_scan_dir(fs, root, &build);
  if (ok && build.count) {
    qsort(build.entries, build.count, sizeof(StartupHotIndexEntry),
          startup_hot_entry_compare);
    size_t out = 1;
    for (size_t i = 1; i < build.count; ++i) {
      if (strcasecmp(build.entries[i].path, build.entries[out - 1].path)) {
        if (out != i) build.entries[out] = build.entries[i];
        out++;
      } else {
  ...
  if (ok) {
    g_startup_hot_index = build.entries;
    g_startup_hot_index_count = build.count;
    __atomic_store_n(&g_startup_hot_index_state, 2, __ATOMIC_RELEASE);
  } else {
    startup_hot_index_build_free(&build);
    __atomic_store_n(&g_startup_hot_index_state, -1, __ATOMIC_RELEASE);
  }
  ...
  if (pvz2_startup_profile_active()) {
    const u64 hz = armGetSystemTickFreq();
    const u64 ticks = armGetSystemTick() - begin;
    const unsigned long long ms = hz ?
        (unsigned long long)((ticks * 1000ULL) / hz) : 0;
    debugPrintf("STARTUPPROF +%llums HOT-INDEX build=%s duration=%llums entries=%lu files=%lu dirs=%lu\n",
                pvz2_startup_profile_elapsed_ms(), ok ? "ok" : "failed", ms,
                (unsigned long)(ok ? g_startup_hot_index_count : 0),
                (unsigned long)build.files, (unsigned long)build.directories);
  }
#endif
  return ok;
}

static int startup_hot_index_ready(FsFileSystem *fs) {
  int state = __atomic_load_n(&g_startup_hot_index_state, __ATOMIC_ACQUIRE);
  if (state == 2) return 1;
  if (state == -1 || state == 1) return 0;
  int expected = 0;
  if (!__atomic_compare_exchange_n(&g_startup_hot_index_state, &expected, 1,
                                   0, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE))
    return expected == 2;
  return startup_hot_index_build(fs);
}

static const StartupHotIndexEntry *startup_hot_index_find(const char *fs_path) {
  size_t lo = 0, hi = g_startup_hot_index_count;
  while (lo < hi) {
    const size_t mid = lo + (hi - lo) / 2;
    const int cmp = strcasecmp(g_startup_hot_index[mid].path, fs_path);
    if (cmp < 0) lo = mid + 1;
    else hi = mid;
  }
  if (lo < g_startup_hot_index_count &&
      !strcasecmp(g_startup_hot_index[lo].path, fs_path))
    return &g_startup_hot_index[lo];
  return NULL;
}

/* Returns 1 when the exact startup stat was handled from the snapshot. */
static int startup_hot_index_stat(const char *path, struct bionic_stat *out,
                                  int *result) {
  static const char hot_root[] = DATA_DIR "/No_Backup/hotpackagestorage";
  const size_t hot_len = sizeof(hot_root) - 1;
  if (!path || strncmp(path, hot_root, hot_len) ||
      (path[hot_len] != '\0' && path[hot_len] != '/')) return 0;
  if (pvz2_startup_profile_loading_state() != 3) return 0;

  FsFileSystem *fs = fsdevGetDeviceFileSystem("sdmc");
  if (!fs || !startup_hot_index_ready(fs)) return 0;
  const char *fs_path = !strncmp(path, "sdmc:", 5) ? path + 5 : path;
  const StartupHotIndexEntry *entry = startup_hot_index_find(fs_path);
  if (!entry) {
    errno = ENOENT;
    *result = -1;
    ++g_startup_hot_index_misses;
#if PVZ2_ENABLE_STARTUP_PROFILER
    const unsigned misses = g_startup_hot_index_misses;
    if (pvz2_startup_profile_active() &&
        (misses == 1 || misses == 1024 || misses == 2048 ||
         misses == 3072 || misses == 4096))
      debugPrintf("STARTUPPROF +%llums HOT-INDEX misses=%u hits=%u\n",
                  pvz2_startup_profile_elapsed_ms(), misses,
                  g_startup_hot_index_hits);
#endif
    return 1;
  }
  ...
  out->st_nlink = 1;
  out->st_blksize = 4096;
  out->st_mode = entry->type == FsDirEntryType_Dir ?
      (S_IFDIR | 0777) : (S_IFREG | 0666);
  *result = 0;
  ++g_startup_hot_index_hits;
  return 1;
}

/* Content metadata probes are extremely hot during title loading.  Routing
 * those probes through newlib stat() forces them through g_file_lock, which is
 * also shared by open/close operations from worker threads.  The game can then
 * spend seconds blocked on a simple missing-file check.  Query the already
 * mounted SD filesystem through libnx/fsp-srv instead; this does not touch
 * newlib's fd/device table and therefore does not need g_file_lock. */
static int direct_content_stat(const char *path, struct bionic_stat *out,
                               int *handled) {
  static const char hot_root[] = DATA_DIR "/No_Backup/hotpackagestorage";
  static const char cdn_root[] = DATA_DIR "/No_Backup/CDN.13.3";
  *handled = 0;
  if (!path || strncmp(path, "sdmc:", 5) != 0) return -1;
  const size_t hot_len = sizeof(hot_root) - 1;
  const size_t cdn_len = sizeof(cdn_root) - 1;
  const int in_hot = !strncmp(path, hot_root, hot_len) &&
                     (path[hot_len] == '\0' || path[hot_len] == '/');
  const int in_cdn = !strncmp(path, cdn_root, cdn_len) &&
                     (path[cdn_len] == '\0' || path[cdn_len] == '/');
  if (!in_hot && !in_cdn) return -1;
  ...
  *handled = 1;
  ...
  const char *fs_path = path + 5; /* strip the sdmc: mount prefix */
  FsDirEntryType type;
#if PVZ2_ENABLE_STARTUP_PROFILER
  const u64 begin = armGetSystemTick();
#endif
  Result rc = fsFsGetEntryType(fs, fs_path, &type);
  if (R_FAILED(rc)) {
    errno = ENOENT;
#if PVZ2_ENABLE_STARTUP_PROFILER
    if (pvz2_startup_profile_active()) {
      ...
      const unsigned long long ms =
          (unsigned long long)((ticks * 1000ULL) / armGetSystemTickFreq());
      if (ms >= 25)
        debugPrintf("STARTUPPROF +%llums SLOW-METADATA duration=%llums result=missing path=%s\n",
                    pvz2_startup_profile_elapsed_ms(), ms, fs_path);
    }
#endif
    return -1;
  }
  ...
  if (R_SUCCEEDED(fsFsGetFileTimeStampRaw(fs, fs_path, &ts)) && ts.is_valid) {
    out->st_atim.tv_sec = (int64_t)ts.accessed;
    out->st_mtim.tv_sec = (int64_t)ts.modified;
    out->st_ctim.tv_sec = (int64_t)ts.created;
  }
  ...
  if (pvz2_startup_profile_active()) {
    ...
    const unsigned long long ms =
        (unsigned long long)((ticks * 1000ULL) / armGetSystemTickFreq());
    if (ms >= 25)
      debugPrintf("STARTUPPROF +%llums SLOW-METADATA duration=%llums result=found path=%s\n",
                  pvz2_startup_profile_elapsed_ms(), ms, fs_path);
  }
#endif
  return 0;
...
int stat_fake(const char *path, struct bionic_stat *st) {
  ...
  const uintptr_t caller_off = pvz2_startup_profile_native_offset(caller);
  const int exact_startup_stat = pvz2_startup_profile_active() &&
                                 caller_off == 0x240507c &&
                                 pvz2_startup_profile_loading_state() == 3;
  const u64 exact_stat_begin = exact_startup_stat ? armGetSystemTick() : 0;
#endif
  if (path_would_crash_newlib(path)) {
    errno = ENOENT;
    return -1;
  ...
  const uintptr_t startup_caller = (uintptr_t)__builtin_return_address(0);
  const uintptr_t startup_caller_off =
      pvz2_startup_profile_native_offset(startup_caller);
  if (startup_caller_off == 0x240507c &&
      pvz2_startup_profile_loading_state() == 3) {
    int indexed_result = -1;
    if (startup_hot_index_stat(path, st, &indexed_result)) return indexed_result;
  }

  int handled = 0;
  const int direct = direct_content_stat(path, st, &handled);
  if (handled) {
#if PVZ2_ENABLE_STARTUP_PROFILER
    if (exact_startup_stat) {
      const u64 ticks = armGetSystemTick() - exact_stat_begin;
      const u64 hz = armGetSystemTickFreq();
      const unsigned long long ms = hz ? (unsigned long long)((ticks * 1000ULL) / hz) : 0;
      if (ms >= 25)
        debugPrintf("STARTUPPROF +%llums STAT-SLOW duration=%llums caller=+0x%lx route=direct result=%d path=%s\n",
                    pvz2_startup_profile_elapsed_ms(), ms,
                    (unsigned long)caller_off, direct, path ? path : "(null)");
    }
#endif
    return direct;
  }
  ...
  if (exact_startup_stat) {
    ...
    const unsigned long long ms = hz ? (unsigned long long)((ticks * 1000ULL) / hz) : 0;
    if (ms >= 25)
      debugPrintf("STARTUPPROF +%llums STAT-SLOW duration=%llums caller=+0x%lx route=newlib result=%d path=%s\n",
                  pvz2_startup_profile_elapsed_ms(), ms,
                  (unsigned long)caller_off, ret, path ? path : "(null)");
  }
#endif
  return ret;
}
...
// Optional I/O timing instrumentation. Sampling the clock around every read is
// measurable hot-path work, so normal builds compile it out.
...
size_t fread_fake(void *ptr, size_t size, size_t n, FILE *f) {
  ...
  StartupIoFile *entry = startup_io_file_for_fd(fd);
  const u64 begin = entry ? armGetSystemTick() : 0;
  const size_t result = fread(ptr, size, n, f);
  if (entry) {
    __atomic_fetch_add(&entry->fread_calls, 1u, __ATOMIC_RELAXED);
    __atomic_fetch_add(&entry->fread_bytes,
                       (unsigned long long)(result * size), __ATOMIC_RELAXED);
    __atomic_fetch_add(&entry->fread_ticks, armGetSystemTick() - begin,
                       __ATOMIC_RELAXED);
  }
  return result;
}
...
int fseeko_fake(FILE *f, off_t off, int whence) {
  ...
  StartupIoFile *entry = startup_io_file_for_fd(fileno(f));
  const u64 begin = entry ? armGetSystemTick() : 0;
  const int result = fseeko(f, off, whence);
  if (entry) {
    __atomic_fetch_add(&entry->seek_calls, 1u, __ATOMIC_RELAXED);
    __atomic_fetch_add(&entry->seek_ticks, armGetSystemTick() - begin,
                       __ATOMIC_RELAXED);
...
int fseek_fake(FILE *f, long off, int whence) {
  ...
  StartupIoFile *entry = startup_io_file_for_fd(fileno(f));
  const u64 begin = entry ? armGetSystemTick() : 0;
  const int result = fseek(f, off, whence);
  if (entry) {
    __atomic_fetch_add(&entry->seek_calls, 1u, __ATOMIC_RELAXED);
    __atomic_fetch_add(&entry->seek_ticks, armGetSystemTick() - begin,
                       __ATOMIC_RELAXED);
...
static const char *remap_data_path(const char *path, char *buf, size_t bufsz) {
   ...
   * tar.full additionally needs Linux unlink-on-open semantics; map its one
   * logical name onto a rotating physical generation. */
  if (!strcmp(path, CDN_TAR_FULL_ANDROID)) {
    const unsigned generation = cdn_tar_full_generation_load();
    snprintf(buf, bufsz, DATA_DIR CDN_TAR_FULL_ANDROID ".pvz2.%u", generation);
    return buf;
  }
...
int rename_fake(const char *oldp, const char *newp) {
  ...
  FILE_UNLOCK();
  if (PVZ2_ENABLE_VERBOSE_RUNTIME_LOG || rc != 0 ||
      path_is_local_profiles(rnew) || path_is_java_properties(rnew))
    debugPrintf("rename(%s -> %s) -> %d\n", rold, rnew, rc);
  return rc;
}

/* The HTTP payload is authoritative, but doing a
 * full synchronous install inside http_complete() stalled curl for 5-28 s per
 * archive and made main.tar hit its 30 s timeout. Queue only the two packages
 * Arena needs (main + levels_joust), let the whole CDN HTTP batch drain, then
 * install. main.tar is filtered to Arena/Joust configuration; levels_joust is
 * filtered to levels/joust/. This keeps stock HTTP callbacks and CDN state
 * authoritative while avoiding thousands of unrelated SD-card file creates. */


int remove_fake(const char *path) {
  const int tar_full_logical = path_is_cdn_tar_full_logical(path);
  char a[600];
  const char *p = remap_data_path(path, a, sizeof a);
  if (path_would_crash_newlib(p)) return -1;
  const int physical_rc = remove_L(p);

  if (tar_full_logical) {
    cdn_tar_full_generation_advance();
    /* Whether Horizon managed to delete the backing file or refused because
     * libtar still has it open, the logical name is now gone: subsequent
     * opens map to the next generation, matching POSIX unlink semantics. */
    return 0;
  }
...
FILE *fopen_fake(const char *path, const char *mode) {
  ...
  if (f && strchr(mode, 'r')) {
    int buffered = 0;
    const char *ext = strrchr(path, '.');
    if (startup_io_path_should_buffer(path)) {
      if (setvbuf(f, NULL, _IOFBF, STARTUP_IO_BUFFER_SIZE) == 0) {
        buffered = 1;
#if PVZ2_ENABLE_STARTUP_PROFILER
        __atomic_fetch_add(&g_startup_io_buffered_streams, 1u, __ATOMIC_RELAXED);
#endif
      }
    } else if (ext && (strcasecmp(ext, ".ras") == 0 ||
                       strcasecmp(ext, ".msf") == 0)) {
      setvbuf(f, NULL, _IOFBF, 128 * 1024);
    }
    startup_io_register_fd(fileno(f), path, buffered);
  }
  return f;
}
...
static void asset_missing_log(const char *path, const char *tried) {
  ...
  if (call <= 8 || (call % 1024) == 0
      )
    debugPrintf("joust: open(%s) MISSING (tried %s) call=%u\n",
                path ? path : "?", tried ? tried : "?", call);
}

#if PVZ2_ENABLE_STARTUP_PROFILER
static u64 g_startup_asset_open_ticks;
static unsigned g_startup_asset_open_count;
#endif

static void startup_profile_asset_open(const char *path, const char *source,
                                       u64 begin) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (!pvz2_startup_profile_active()) return;
  const u64 hz = armGetSystemTickFreq();
  const u64 elapsed_ticks = armGetSystemTick() - begin;
  const unsigned long long ms = hz
      ? (unsigned long long)(elapsed_ticks * 1000u / hz) : 0;
  g_startup_asset_open_count++;
  g_startup_asset_open_ticks += elapsed_ticks;
  if (ms >= 25)
    debugPrintf("STARTUPPROF +%llums ASSET-OPEN-SLOW duration=%llums source=%s path=%s\n",
                pvz2_startup_profile_elapsed_ms(), ms,
                source ? source : "?", path ? path : "?");
#else
  (void)path; (void)source; (void)begin;
#endif
}

void libc_startup_profile_snapshot(const char *tag) {
#if PVZ2_ENABLE_STARTUP_PROFILER
  if (!pvz2_startup_profile_active()) return;
  const u64 hz = armGetSystemTickFreq();
  const unsigned long long asset_ms = hz
      ? (unsigned long long)((g_startup_asset_open_ticks * 1000ULL) / hz) : 0;
  debugPrintf("STARTUPPROF +%llums FILE-SUMMARY tag=%s assets=%u/%llums hot_index=%lu hits=%u misses=%u\n",
              pvz2_startup_profile_elapsed_ms(), tag ? tag : "?",
              g_startup_asset_open_count, asset_ms,
              (unsigned long)g_startup_hot_index_count,
              g_startup_hot_index_hits, g_startup_hot_index_misses);
  unsigned fread_calls = 0, read_calls = 0, seek_calls = 0;
  unsigned long long fread_bytes = 0, read_bytes = 0;
  u64 fread_ticks = 0, read_ticks = 0, seek_ticks = 0;
  for (unsigned i = 0; i < g_startup_io_file_count; ++i) {
    StartupIoFile *entry = &g_startup_io_files[i];
    fread_calls += __atomic_load_n(&entry->fread_calls, __ATOMIC_RELAXED);
    read_calls += __atomic_load_n(&entry->read_calls, __ATOMIC_RELAXED);
    seek_calls += __atomic_load_n(&entry->seek_calls, __ATOMIC_RELAXED);
    fread_bytes += __atomic_load_n(&entry->fread_bytes, __ATOMIC_RELAXED);
    read_bytes += __atomic_load_n(&entry->read_bytes, __ATOMIC_RELAXED);
    fread_ticks += __atomic_load_n(&entry->fread_ticks, __ATOMIC_RELAXED);
    read_ticks += __atomic_load_n(&entry->read_ticks, __ATOMIC_RELAXED);
    seek_ticks += __atomic_load_n(&entry->seek_ticks, __ATOMIC_RELAXED);
  }
  const unsigned long long io_ms = hz
      ? (unsigned long long)((fread_ticks + read_ticks + seek_ticks) * 1000ULL / hz)
      : 0;
  debugPrintf("STARTUPPROF +%llums RESOURCE-IO-SUMMARY tag=%s files=%u buffered=%u fread=%u/%llubytes read=%u/%llubytes seek=%u io_ms=%llums\n",
              pvz2_startup_profile_elapsed_ms(), tag ? tag : "?",
              g_startup_io_file_count, g_startup_io_buffered_streams,
              fread_calls, fread_bytes, read_calls, read_bytes, seek_calls,
              io_ms);
#else
  (void)tag;
#endif
...
void *AAssetManager_open_fake(void *mgr, const char *path, int mode) {
  ...
  (void)mode;
  if (!path) return NULL;
  u64 startup_asset_open_tick = 0;
#if PVZ2_ENABLE_STARTUP_PROFILER
  startup_asset_open_tick = armGetSystemTick();
#endif
  ...
  if (memory) {
    ...
    startup_profile_asset_open(path, source, startup_asset_open_tick);
    return asset;
  }
  ...
  if (!memory) {
    ...
    startup_profile_asset_open(path, "missing", startup_asset_open_tick);
    return NULL;
  }
  ...
  startup_profile_asset_open(path, "loose", startup_asset_open_tick);
  return asset;
}
...
int AAsset_read_fake(void *asset, void *buf, size_t count) {
  ...
  const int result = (int)fread(buf, 1, count, a->f);
  return result;
}
...
static FakeRwLock *get_rwlock(void **storage) {
  ...
  FakeRwLock *lock = __atomic_load_n((FakeRwLock **)storage, __ATOMIC_ACQUIRE);
  if (lock) return lock;

  FakeRwLock *created = calloc(1, sizeof(*created));
  if (!created) return NULL;
  rwlockInit(&created->lock);

  FakeRwLock *expected = NULL;
  if (!__atomic_compare_exchange_n((FakeRwLock **)storage, &expected, created,
...
int pthread_rwlock_unlock_fake(void **rw) {
  if (!rw) return EINVAL;
  FakeRwLock *lock = __atomic_load_n((FakeRwLock **)rw, __ATOMIC_ACQUIRE);
  if (!lock) return EINVAL;
  // libnx needs to know which way it was locked
  if (rwlockIsWriteLockHeldByCurrentThread(&lock->lock))
    rwlockWriteUnlock(&lock->lock);
  else
    rwlockReadUnlock(&lock->lock);
  return 0;
}
